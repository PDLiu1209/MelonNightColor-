...
from models import (
    ...
    UNet_BN,
    UNet_E4_ECA,
    UNet_Decoder_Pyramid,
    UNet_BN_ECA,
    UNet_BN_DecoderPyramid,
    UNet_ECA_DecoderPyramid,
    UNet_BN_ECA_DecoderPyramid,
)
...
def build_model(name: str, num_classes: int, default_model_config: dict):
    ...
    if name == 'SwinUnet':
        ...
        return UNet_BN(input_channels=3, num_classes=num_classes)
    elif name == 'UNet_E4_ECA':
        return UNet_E4_ECA(input_channels=3, num_classes=num_classes)
    elif name == 'UNet_Decoder_Pyramid':
        return UNet_Decoder_Pyramid(input_channels=3, num_classes=num_classes)
    elif name == 'UNet_BN_ECA':
        return UNet_BN_ECA(input_channels=3, num_classes=num_classes)
    elif name == 'UNet_BN_DecoderPyramid':
        return UNet_BN_DecoderPyramid(input_channels=3, num_classes=num_classes)
    elif name == 'UNet_ECA_DecoderPyramid':
        return UNet_ECA_DecoderPyramid(input_channels=3, num_classes=num_classes)
    elif name == 'UNet_BN_ECA_DecoderPyramid':
        return UNet_BN_ECA_DecoderPyramid(input_channels=3, num_classes=num_classes)
    else:
...
def main(args):
    if args.save_dir:
        ...
        # 添加时间戳 + SLURM job ID（或进程 ID）到保存路径，避免并发 job 写入同一目录
        current_time = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
        job_id = os.environ.get('SLURM_JOB_ID', str(os.getpid()))
        save_path = f'./run_results/{current_time}_{job_id}_{args.network}_{args.data}'
    ...
    train_path = f'/public/home/liupengdong2024/Paper_Two/Dataset_5474/{dataset_name}/train'
    val_path = f'/public/home/liupengdong2024/Paper_Two/Dataset_5474/{dataset_name}/val'
    test_path = f'/public/home/liupengdong2024/Paper_Two/Dataset_5474/{dataset_name}/test'
    num_classes = 3
    print(f"[num_classes] {num_classes}")

    trainDataset = MyDataset(imgs_path=train_path,  transform=train_tf, normalize_to_01=False)
    valDataset   = MyDataset(imgs_path=val_path,    transform=val_tf, normalize_to_01=False)
    testDataset  = MyDataset(imgs_path=test_path,   transform=test_tf, normalize_to_01=False)
    num_workers = min([os.cpu_count(), args.batch_size if args.batch_size > 1 else 0, 8])
    print('Using %g dataloader workers' % num_workers)

    # DeepLab 模型的 ASPP 模块在 batch_size=1 时会报错，需要丢弃最后不足的 batch
    drop_last = (args.network.lower() == 'deeplab')
    trainLoader = DataLoader(trainDataset, batch_size=args.batch_size, num_workers=num_workers, shuffle=True, drop_last=drop_last)
    ...
    if args.opt is not None:
        configs.opt = args.opt
    if args.sch is not None:
        configs.sch = args.sch
    if args.lr is not None:
        configs.lr = args.lr
    if args.warmup is not None:
        configs.warm_up_epochs = args.warmup
    if args.wd is not None:
        configs.weight_decay = args.wd
    ...
    if args.loss == 'CE':
        stem_w = args.stem_weight if args.stem_weight is not None else 3.0
        ls    = args.label_smoothing if args.label_smoothing is not None else 0.3
        bce_pos_weight = torch.tensor([1.0] * (num_classes - 1) + [stem_w], dtype=torch.float32).to(device)
        criterion = torch.nn.CrossEntropyLoss(weight=bce_pos_weight, label_smoothing=ls)

    elif args.loss == 'BCDE':
        criterion = BceDiceLoss(wb=1, wd=1)

    elif args.loss == 'CEDL':
        dw = args.dice_weight if args.dice_weight is not None else 0.5
        criterion = CeDiceLoss(num_classes, loss_weight=[1 - dw, dw])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', default=True, type=lambda x: x.lower() not in ('false', '0', 'no'), help="True: 训练+测试；False: 仅加载 best_model 测试   传入True或者什么都不传就是训练加测试   传入False或者0就是仅测试")
    ...
    parser.add_argument('--loss', default='CE', type=str, choices=['CE', 'BCDE', 'CEDL'])
    parser.add_argument('--opt', default=None, type=str, help="覆盖 setting.py 的 configs.opt，不传则使用 setting.py 默认值")
    parser.add_argument('--data', default='data_white', type=str)
    parser.add_argument('--sch', default=None, type=str, help="覆盖 setting.py 的 configs.sch，不传则使用 setting.py 默认值")
    parser.add_argument('--lr', default=None, type=float, help="覆盖 setting.py 的 configs.lr，不传则使用 setting.py 默认值")
    parser.add_argument('--warmup', default=None, type=int, help="覆盖 setting.py 的 configs.warm_up_epochs，不传则使用 setting.py 默认值")
    parser.add_argument('--wd', default=None, type=float, help="覆盖 setting.py 的 configs.weight_decay，不传则使用 setting.py 默认值")
    parser.add_argument('--stem-weight', default=None, type=float, help="CE Loss 中茎部类别权重，默认 3.0")
    parser.add_argument('--label-smoothing', default=None, type=float, help="CE Loss 标签平滑系数，默认 0.3")
    parser.add_argument('--dice-weight', default=None, type=float, help="CEDL 中 Dice 占比，默认 0.5（CE 占 1-dice_weight）")
