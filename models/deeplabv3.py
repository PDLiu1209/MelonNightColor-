...
from models.deeplabv3_modules.xception import xception
from models.deeplabv3_modules.mobilenetv2 import mobilenetv2
from models.amp_utils import process
    ...
    def forward(self, x):
        ...
        if not self.fix_amp:
            if torch.sum(self.running_amp) == 0:
                ...
                self.running_amp.copy_(torch.from_numpy(amp))
            else:
                x, amp = process(x.cpu().numpy(), self.running_amp.cpu().numpy(), self.momentum, self.fix_amp)
                self.running_amp.copy_(torch.from_numpy(amp))
        else:
...
class MobileNetV2(nn.Module):
    def __init__(self, downsample_factor=8, pretrained=True):
        ...
        self.features = model.features[:-1]  # 去掉最后的1×1卷积，相当于到mobileNet2.py的列表循环为止-91

        # 获得每一个下采样所处于的位置
        self.total_idx = len(self.features)
        self.down_idx = [2, 4, 7, 14]

        if downsample_factor == 8:  # 如果下采样的倍数是8的话只会进行三次下采样，把倒数两次的步长为2的brock步长变为1,膨胀系数dilate进行修改
...
class DeepLab(nn.Module):
    def __init__(self, num_classes, input_channels=3, input_size=224):
