...
def train_one_epoch(model, optim, train_loader, test_loader, device, loss_fuc, num_classes, validate=False):
    ...
    for train_image, train_target in tqdm(train_loader, leave=False, desc="Train"):
        ...
        if isinstance(criterion, CrossEntropyLoss) or isinstance(criterion, CeDiceLoss):
            # CE / CeDiceLoss：直接 logits + int64 标签（CeDiceLoss 内部自处理 softmax）
            loss = criterion(output, train_target)
        else:
            # BceDiceLoss：需要 prob + one-hot
    ...
    if validate:
        ...
        with torch.inference_mode():  # 比 no_grad 更快
            for test_image, test_target in tqdm(test_loader, leave=False, desc="Val"):
                ...
                # 验证损失：CE / CeDiceLoss 直接用 logits，BceDiceLoss 需 one-hot
                if isinstance(criterion, CrossEntropyLoss) or isinstance(criterion, CeDiceLoss):
...
def plot_lr_decay(scheduler, optimizer, epochs, LR_path):
    ...
    if isinstance(scheduler, ReduceLROnPlateau):
        print("[plot_lr_decay] ReduceLROnPlateau 依赖运行时指标，跳过 LR 曲线绘制")
        return
    # 备份真实状态
...
def binary_hausdorff95(s, g, spacing=None):
    ...
    if not dist_list1 or not dist_list2:    
        return float('nan')
...
def compute_hd95_per_class(pred_np: np.ndarray,
    ...
    for c in range(num_classes):
        ...
        tgt_c  = ((tgt  == c) & valid).astype(np.uint8)
        if pred_c.any() and tgt_c.any():
            try:
                out[c] = _medpy_hd95(pred_c, tgt_c, voxelspacing=spacing)
            except Exception:
                out[c] = np.nan
    return out
...
def append_log_to_excel(log, model_name, categories, out_path):
    ...
    cols = ["model","class","OA","mIoU","precision","recall","IoU","mDice","Dice","hd95","hd95_macro","AP","mAP","FPS"]
    df_new = df_new[cols]
